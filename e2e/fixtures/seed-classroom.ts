...
/** The stage every spec that uses this fixture navigates to. */
export const TEST_STAGE_ID = 'e2e-test-stage';

const SETTINGS_STORAGE = createSettingsStorage({ sidebarCollapsed: false });

/**
 * Seeds IndexedDB with one stage and three slide scenes using the raw
 * IndexedDB API, then leaves the browser on the homepage.
 *
 * Shared by the classroom behaviour specs and the appearance (screenshot)
 * spec: both need a classroom that exists without going through generation,
 * and a second copy of this would drift from the first the moment the Dexie
 * schema moves.
 */
export async function seedClassroom(page: import('@playwright/test').Page) {
